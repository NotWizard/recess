...
- **休息浮窗**：居中、置顶、不抢焦点；窗口标题 `Recess`、正文大标题 `该休息了`；按钮分主次——`开始休息`=蓝底白字主按钮（显式填充，不受非活动窗口影响），`跳过`=浅灰次按钮；可直接关闭；关闭后菜单栏图标保留入口，可再次激活休息。固定内容尺寸 360×200，避免 `NSHostingController` 按 fitting size 压成竖条。
